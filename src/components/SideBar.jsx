...
const SideBar = () => {
  ...
  const handleConfirmLogout = () => { handleCloseLogoutConfirm(); logout(); };
  ...
  const handleProfileSaveChanges = async () => {
    ...
    const isEmailDuplicate = users.some(u => u.email.toLowerCase() === trimmedEmail.toLowerCase() && u.id !== user.id);
  ...
  const visibleItems = useMemo(() => {
    if (user && user.role === 'admin') return sidebarItems;
  ...
  return (
    <>
      ...
      <button className="sidebar-toggle-btn-apple" onClick={toggleSidebar}><BsList /></button>
        ...
        <div className="sidebar-footer-apple">
          <Dropdown drop="up" className="w-100">
            ...
            <Dropdown.Menu className="dropdown-menu-apple">
              <Dropdown.Item onClick={handleShowProfileModal}><BsPencilFill className="me-2" /> Editar Perfil</Dropdown.Item>
              <Dropdown.Divider />
      ...
      <Modal show={showLogoutConfirm} onHide={handleCloseLogoutConfirm} centered dialogClassName="apple-modal-dark">
        <Modal.Header closeButton>
          <Modal.Title className="modal-title-apple"><BsExclamationTriangleFill className="text-warning me-2" />Confirmar Cierre de Sesión</Modal.Title>
        </Modal.Header>
        <Modal.Body>¿Estás seguro, <strong>{displayName}</strong>, que deseas cerrar sesión?</Modal.Body>
        <Modal.Footer>
          <Button variant="secondary" className="apple-button-secondary" onClick={handleCloseLogoutConfirm}>Cancelar</Button>
          <Button variant="danger" className="apple-button-danger" onClick={handleConfirmLogout}>Cerrar Sesión</Button>
        </Modal.Footer>
      </Modal>
      <Modal show={showProfileModal} onHide={handleCloseProfileModal} centered dialogClassName="apple-modal-dark">
        <Modal.Header closeButton><Modal.Title className="modal-title-apple">Editar Perfil</Modal.Title></Modal.Header>
        <Modal.Body>
            <AppleStyleAlert message={alertMessage} onClose={() => setAlertMessage('')} />
            <Form>
                <Form.Group className="mb-3"><Form.Label>Nombre Completo</Form.Label><Form.Control type="text" name="name" value={profileData.name} onChange={handleProfileFormChange} /></Form.Group>
                <Form.Group className="mb-3"><Form.Label>Email</Form.Label><Form.Control type="email" name="email" value={profileData.email} onChange={handleProfileFormChange} /></Form.Group>
                <hr className="my-4" />
                <p className="text-muted small">Dejar los siguientes campos en blanco si no deseas cambiar la contraseña.</p>
                <Row>
                  <Col><Form.Group className="mb-3"><Form.Label>Nueva Contraseña</Form.Label><Form.Control type="password" name="password" value={profileData.password} onChange={handleProfileFormChange} /></Form.Group></Col>
                  <Col><Form.Group className="mb-3"><Form.Label>Confirmar Contraseña</Form.Label><Form.Control type="password" name="confirmPassword" value={profileData.confirmPassword} onChange={handleProfileFormChange} /></Form.Group></Col>
                </Row>
            </Form>
        </Modal.Body>
        <Modal.Footer>
          <Button variant="secondary" className="apple-button-secondary" onClick={handleCloseProfileModal}>Cancelar</Button>
          <Button variant="primary" className="apple-button-primary" onClick={handleProfileSaveChanges}>Guardar Cambios</Button>
        </Modal.Footer>
      </Modal>
    </>
  )
